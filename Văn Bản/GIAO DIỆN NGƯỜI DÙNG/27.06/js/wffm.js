(function ($scw, scBeacon) {
    ...
    $scw.widget("wffm.wffmForm", {
        _create: function () {
            ...
            var checkboxes = $scw(form).find('input[type="checkbox"].checkbox-group-required');
            $scw.each(checkboxes, function () {
                var checkboxes_message = $scw(this).attr('data-val-required');
                $scw(this).rules("add", {
                    required: true,
                    minlength: 1,
                    messages: {
                        required: checkboxes_message
                    }
                });
            });

            $scw.validator.addMethod("accept", function (value, element, param) {
                if (param === "")
                    return true;
                if (value !== "") {
            ...
            $scw.validator.addMethod('filesize', function (value, element, param) {
                if (param === "")
                    return true;
                return this.optional(element) || (element.files[0].size <= param);
            });

            var uploadFileFields = $scw(form).find('.osw-uploadfile input[type="file"]');
            $scw.each(uploadFileFields, function () {
                var validFileExtensions = $scw(this).data("extension");
                var validFileExtensionsError = $scw(this).data("errorExtensions");
                var validFileSize = $scw(this).data("filesize");
                var validFileSizeError = $scw(this).data("errorFilesize");
                $scw(this).rules("add", {
                    accept: validFileExtensions,
                    filesize: validFileSize,
                    messages: {
                        accept: validFileExtensionsError,
                        filesize: validFileSizeError
                    }
                });
            });
